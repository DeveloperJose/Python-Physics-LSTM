...
contains
    ...
    subroutine aseq(a_start, a_end, n_step, &
                    a_arr, cl_arr, cd_arr, cm_arr, cp_arr, conv_arr, arr1, arr2, arr3, arr4, arr5, arr6) bind(c, name='aseq')
        use m_xoper, only: specal, viscal, fcpmin
        use i_xfoil
        real(c_float), intent(in) :: a_start, a_end
        integer(c_int), intent(in) :: n_step
        real(c_float), dimension(n_step), intent(inout) :: a_arr, cl_arr, cd_arr, cm_arr, cp_arr, arr1, arr2, arr3, arr4, arr5, arr6
