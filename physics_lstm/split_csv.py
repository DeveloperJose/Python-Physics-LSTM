...
with open(input_path, "r") as file:
    ...
    p_bar = tqdm(
        desc="Splitting big CSV file into individual timesteps",
        postfix="Timestep 0",
        total=get_num_lines(input_path),
    )
    ...
    for row in reader:
        ...
        # Look for the start of a new file
            ...
            if len(data_rows) > 0:
                ...
                np.save(
                    os.path.join(output_dir, f"{current_timestep}"),
                    df[columns_to_save].to_numpy().astype(np.float64),
                )
...
gen = data.SledDataGenerator(
    output_dir,
    sequence_length=1,
    inputs=data.COLUMNS_H,
    outputs=[],
    scaler=None,
    start=plane_timesteps[0],
    end=plane_timesteps[-1],
)

for idx, input_name in enumerate(data.COLUMNS_H):
    d = gen.x_data[:, :, idx].numpy()
    print(
        f"For input [{input_name}] | min={np.min(d):.5f}, max={np.max(d):.5f}, mean={np.mean(d):.5f}, median={np.median(d):.5f}, std={np.std(d):.5f}, var={np.var(d):.5f}"
    )
